...
GRID_SIZE = settings.MEDIUM_GRID_SIZE
...
cell_counter_label = tk.Label(cell_counter_inner, text=f"{Mlogic.Cell.cell_count}", font=('System', 28, 'bold'), 
                       bg='#000080', fg='#ff0000')
cell_counter_label.pack()

# Connect label to Cell class
Mlogic.Cell.cell_count_label_object = cell_counter_label
...
Mlogic.Cell.timer_label_object = timer_label
...
for row in range(settings.MEDIUM_GRID_SIZE):
    for col in range(settings.MEDIUM_GRID_SIZE):
        c = Mlogic.Cell(row, col)
        c.Create_btn_object(board_frame)
        c.cell_btn_object.grid(row=row, column=col, sticky='nsew')

Mlogic.Cell.randomize_mines()

# debug
for c in Mlogic.Cell.all:
    print(c.is_mine)
